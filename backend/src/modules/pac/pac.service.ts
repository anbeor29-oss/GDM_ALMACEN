...
export async function timbrarJson(
  ...
  if (typeof provider.stampFromJson === 'function') {
     ...
     * atrás. Se corta aquí porque el "MODO SIMULACIÓN" del diálogo se lee como
     * un aviso menor y no como lo que es: que ese comprobante no existe para el
     * SAT.
     *
...
export async function cancelarComprobante(
  ...
  const { csd } = await obtenerCsdConAviso(companyId, `cancelación de ${uuid}`);

  logger.info(`Cancelando comprobante ${uuid} (motivo ${motivo}) vía ${provider.name}`);
  return provider.cancel(uuid, rfcEmisor, motivo, credentials, folioSustitucion, csd ?? undefined);
}
