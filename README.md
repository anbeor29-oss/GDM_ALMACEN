...
| **Importar respaldo (NomiPaq)** | ✅ | Nómina → Importar respaldo: trae la nómina histórica de un `.bak` de **CONTPAQ Nóminas** (empleados, periodos y **recibos** con su CFDI). Como el `.bak` es binario de SQL Server, una **herramienta** local lo lee en la PC y deja un **paquete `.zip`**; la pantalla lo **descomprime en el navegador** (`fflate`), muestra el previo y deja **elegir años**. Mapeo verificado contra el respaldo real (nom10001=empleados, nom10004=conceptos con clave SAT, nom10007=movimientos, nom10043=CFDI); **RFC/CURP reconstruidos** (NomiPaq los guarda partidos, sin la fecha), **gravado=importe1 / exento=importe2** (361/361), **neto** del concepto «Neto». Idempotente; todo `origen='CONTPAQ'`. El XML timbrado NO viene en el `.bak`: el recibo se arma de los movimientos y queda TIMBRADO con su UUID |
