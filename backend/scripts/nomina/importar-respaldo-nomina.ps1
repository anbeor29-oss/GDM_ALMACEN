...
function Invoke-Paquete {
  ...
  try {
    ...
    $zip = Join-Path $dest ("NOMINA_" + $stamp + ".zip")
    if (Test-Path $zip) { Remove-Item $zip -Force }
    try { Compress-Archive -Path (Join-Path $export '*.json') -DestinationPath $zip -Force -ErrorAction Stop }
    catch { $dest = [Environment]::GetFolderPath('Desktop'); $zip = Join-Path $dest ("NOMINA_" + $stamp + ".zip"); if (Test-Path $zip) { Remove-Item $zip -Force }; Compress-Archive -Path (Join-Path $export '*.json') -DestinationPath $zip -Force; & $Log "  (No pude escribir junto al .bak; guardé en el Escritorio.)" }
...
$f.Text = 'Importar respaldo de nómina a NEXO'; $f.Size = New-Object Drawing.Size(720, 560); $f.StartPosition = 'CenterScreen'
...
$btnGen = New-Object Windows.Forms.Button; $btnGen.Text = 'Generar el .zip'; $btnGen.Location = New-Object Drawing.Point(165, 197); $btnGen.Width = 190; $btnGen.BackColor = [Drawing.Color]::FromArgb(109, 40, 217); $btnGen.ForeColor = 'White'; $f.Controls.Add($btnGen)
...
$btnGen.Add_Click({
    ...
    try {
      ...
      [Windows.Forms.MessageBox]::Show(("Paquete listo:`n$zip`n`nAhora regresa a NEXO (Nómina -> Importar respaldo) y sube este .zip (paso 2)."), 'Paquete generado')
